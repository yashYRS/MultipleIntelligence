...
def ai(score_variable):
	...
	if games_played < 8 : 
		...
		if levels_done[game_choice_id - 1] == 0 : 
			...
			module_name , game_state = get_info_game(game_choice_id , 1) ## score is the actual score
			for key in curr_state : 	
		...
		module_name , game_state = get_info_game(game_choice_id ,  levels_done[game_choice_id - 1] + 1  ) # -1 since the index is 1 less than actual id
...
def login():
	...
	if login_user:
		if bcrypt.hashpw(request.form['pwd'].encode('utf-8'), login_user['Password']) == login_user['Password']:
			...
			l='yes'
			return render_template('home.html',l=l,name=login_user['First Name'])
		else:
			return "Error"
	else:
		return "Error"
...
def fungames():
	l=getLog()
	return render_template('fungames.html', l=l)

@app.route('/testgame',methods=['GET','POST'])
def testgame():
	l=getLog()
	return render_template('testgame.html', l=l)
...
def startgame():
	...
	return render_template('FirstScreen.html', game=game, l=l)
